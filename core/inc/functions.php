...
function result_data($errno, $message, $data = ''){
    return array("errno" => $errno, "message" => $message, "result" => $data);
}
...
//转换系统附件里的路径
function to_attach_image($path){
    global $_W;
    if(!preg_match('/^http(s)?:\\/\\/.+/',$path)){
        return $_W['siteroot'].'attachment/'.$path;
    }
    return $path;
}
...
if (!(function_exists('m'))) 
{
	function m($name = '') 
	{
        ...
        require_once NETS_HAOJIK_PATH . '/defines.php';
		require_once $model;
...
if (!(function_exists("mobileUrl"))) 
{
	function mobileUrl($do = '', $query = NULL, $full = false) 
	{
		...
		$query = array_merge(array('m' => 'qt_shop'), $query);
		if (empty($query['mid'])) 
		{
			$mid = intval($_GPC['mid']);
			if (!(empty($mid))) 
...
function payWeixin($openid,$money,$mchid,$tradeno=""){
    ...
    $settings['tj_amount']=$money*100;//money是负数//1*100; //这里要转换成分
