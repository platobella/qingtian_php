...
function _login($forward = '') {
	...
	if (!empty($record)) {
		...
		$diylogin_set = pdo_fetch('select * from '.tablename('qt_diylogin_set').' where username=:username',[':username'=>$_GPC['username']]);

		if($diylogin_set){

            isetcookie('__uniacid', $diylogin_set['uniacid'], 7 * 86400);
            header('location:'.url('site/entry/web',array('m'=>'qt_shop')));
